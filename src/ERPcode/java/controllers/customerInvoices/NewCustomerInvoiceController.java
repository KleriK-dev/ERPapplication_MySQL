...
public class NewCustomerInvoiceController {
    ...
    @FXML
    private Button savePrintButton;
    ...
    @FXML
    private Button vatRegimeSelectionButton;
    ...
    private static NewCustomerInvoiceController instance;

    public NewCustomerInvoiceController(){
        instance = this;
    }

    public static NewCustomerInvoiceController getInstance(){
        return instance;
    }
    ...
    @FXML
    private void ChangeToHandwritedInvoice(ActionEvent event) throws IOException {

        handwriteButton.getScene().getWindow().hide();

        FXMLLoader loader = new FXMLLoader();
        loader.setLocation(getClass().getResource("/fxml/handwritedInvoices/NewHandwritedInvoiceWindow.fxml"));
        Parent root = loader.load();

        //pass user to newHandwritedInvoice Controller
        NewHandwritedInvoiceController newHandwritedInvoiceController = loader.getController();
        newHandwritedInvoiceController.showProgramUser(registrationUser.getText());
        newHandwritedInvoiceController.displayInvoiceID(commons.getNextID("customer_invoices", "id"));

        Stage newHandwritedCustomerInvoiceStage = new Stage();
        Scene newHandwritedCustomerInvoiceScene = new Scene(root);
        Image image = new Image(getClass().getResourceAsStream("/images/logo.png"));
        newHandwritedCustomerInvoiceStage.getIcons().add(image);
        newHandwritedCustomerInvoiceStage.setX(203);
        newHandwritedCustomerInvoiceStage.setY(106);
        newHandwritedCustomerInvoiceStage.setScene(newHandwritedCustomerInvoiceScene);
        newHandwritedCustomerInvoiceStage.setTitle("Customer invoice (New invoice)");
        newHandwritedCustomerInvoiceStage.setAlwaysOnTop(true);
        newHandwritedCustomerInvoiceStage.setResizable(false);
        newHandwritedCustomerInvoiceStage.show();
    ...
    public void OpenAddItemWindow(ActionEvent event) throws IOException {
        ...
        if(customer_id.getText().equals("")){
            alert.show("Warning", "First select a customer, then add items!", Alert.AlertType.WARNING);
        } else {
            FXMLLoader loader = new FXMLLoader();
            loader.setLocation(getClass().getResource("/fxml/customerInvoices/AddItemWindow.fxml"));
            Parent root = loader.load();

            AddItemController addItemController = loader.getController();
            addItemController.loadDataForTransaction(customer_id.getText(), id.getText(), dateTextfield.getText(), timeTextfield.getText());
    ...
    private void OpenEditItemWindow(ActionEvent event) throws IOException {
        ...
        if(index <= -1){
            ...
            FXMLLoader loader = new FXMLLoader();
            loader.setLocation(getClass().getResource("/fxml/customerInvoices/EditItemWindow.fxml"));
            Parent root = loader.load();

            EditItemController editItemController = loader.getController();
            editItemController.getData(table.getSelectionModel().getSelectedItem());
            editItemController.loadDataForTransaction(customer_id.getText(), id.getText(), dateTextfield.getText(), timeTextfield.getText());
    ...
    @FXML
    private void SaveAndPrintInvoice(ActionEvent event) { // STILL TO DO THE CHECKS AND PRINT
        ...
        if(invoiceCustomerID == null || invoiceType == null || invoicePayingWayID == null){ // check for null pointers, if there is no null then go for other checks
            ...
            if (oblist.isEmpty()) {
                ...
                if (invoice.checkBeforeInsertion(invoiceTypeDescription, invoiceType, invoiceLicensePLate, invoiceCustomerID)) {
                    ...
                    Stage customerStage = (Stage) savePrintButton.getScene().getWindow();
                    customerStage.close();
    ...
    private void SearchCustomer(ActionEvent event) throws IOException {
        ...
        customerSelectionController.openedFrom("from new customer invoice");
    ...
    private void SelectInvoiceType(ActionEvent event) throws IOException {
        ...
        invoiceTypeSelectionController.openedFrom("from new customer invoice");
    ...
    private void SelectPayingWay(ActionEvent event) throws IOException {
        ...
        payingWaySelectionController.openedFrom("from new customer invoice");
    ...
    private void SelectVatRegime(ActionEvent event) throws IOException {
        ...
        vaTregimeSelectionController.openedFrom("from new customer invoice");
    ...
    public void getInvoiceTypeData(String invoiceType_id, String invoiceType, String deliveryCheck){
        ...
        String query = "SELECT number FROM invoicenumbering_" + invoiceType_id + " ORDER BY number DESC LIMIT 1 ";
        Statement st;
        ResultSet rs;
